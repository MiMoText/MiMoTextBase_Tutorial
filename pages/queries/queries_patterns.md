...
prefix mmd:<http://data.mimotext.uni-trier.de/entity/>
prefix mmdt:<http://data.mimotext.uni-trier.de/prop/direct/> 
SELECT DISTINCT ?bgrf ?item ?authorlabel ?itemLabel ?year ?narrpers ?tonality ?pages ?dist_format ?normalized WHERE {
 ...
 OPTIONAL { ?item mmdt:P27 ?narrpers} 
 OPTIONAL { ?item mmdt:P31 ?tonality}
 OPTIONAL { ?item mmdt:P25 ?pages}
 OPTIONAL { ?item mmdt:P26 ?dist_format}
 BIND(YEAR(?date) as ?year).
...
PREFIX mmd:<http://data.mimotext.uni-trier.de/entity/>
PREFIX mmdt:<http://data.mimotext.uni-trier.de/prop/direct/>
SELECT ?themeLabel (SAMPLE(?themeLabel) as ?theme)  (count(*) as ?count)
WHERE {
  ?item mmdt:P2 mmd:Q2; # item is instance of literary work
...
PREFIX mmd:<http://data.mimotext.uni-trier.de/entity/>
PREFIX mmdt:<http://data.mimotext.uni-trier.de/prop/direct/>
SELECT ?date ?countNature ?countAll (?countNature / ?countAll AS ?rel) WHERE {
  {
...
```sparql
#title:Thematic statements referenced by ‘topic modeling’ and the bibliography
...
SELECT   ?novelLabel ?topicLabel ?refnode
WHERE
{   
...
PREFIX mmpr: <http://data.mimotext.uni-trier.de/prop/reference/>
PREFIX mmp: <http://data.mimotext.uni-trier.de/prop/>
SELECT ?novel ?novelLabel ?topicLabel ?BGRF_plot_theme
WHERE
{   
    ?novel mmp:P36 ?statement.
    ?statement mmps:P36 ?topic .
...
```sparql
#title:Combinations of narrative Form and intention per year
...
SELECT ?year (COUNT(*) as ?count) ?narrFormTone
  WHERE{
    ...
    ?item mmdt:P38 ?tone. # get tokencount
...
GROUP BY ?narrFormLabel ?year ?toneLabel ?narrFormTone
HAVING (COUNT(*) > 1)
ORDER BY ?year
```
...
1\. 
- as [Tiny-URL](https://tinyurl.com/2avufdfo){:target="_blank"}
...
```
2\. 
- as [Tiny-URL](https://tinyurl.com/23vmyjo6){:target="_blank"}
...
```sparql
# Counts of different narrative perspectives
#defaultView:BarChart
PREFIX mmd:<http://data.mimotext.uni-trier.de/entity/>
PREFIX mmdt:<http://data.mimotext.uni-trier.de/prop/direct/>
...
WHERE {
	 ?item mmdt:P33 ?top . #narrative perspective
